...
"""
mouth_classifier.py
@description cnn+NN models with attention to train the mouth classifier
...
def cropped_mouth_40_lm_sf():
    ...
    cnn_weight = 1.0  # Adjust this weight to decrease the influence of CNN features
...
def cropped_mouth_40_lm_attention():
    # Load the VGG16 model, pretrained on ImageNet, without the top layer
    base_model = VGG16(weights='imagenet', include_top=False, input_shape=(64, 64, 3))
...
def cropped_mouth_124_lm_attention():
    # Load the VGG16 model, pretrained on ImageNet, without the top layer
    base_model = VGG16(weights='imagenet', include_top=False, input_shape=(64, 64, 3))
...
def load_and_prepare_data(csv_path):
    ...
    annotations['Label'] = annotations['Label'].map({'mouth_open': 1, 'mouth_close': 0})  # Adjust mapping as necessary
...
mouth_csv = Config.mouth_csv
image_train, image_test,mouth_train, mouth_test,landmark_124_train, landmark_124_test, landmark_40_train,landmark_40_test, y_train, y_test = load_and_prepare_data(mouth_csv)

model = cropped_mouth_40_lm_sf()
...
checkpoint = ModelCheckpoint(
    '../models/best_model_cropped_mouth_40_lm_sf_new.h5',          # Path where the model will be saved
...
checkpoint = ModelCheckpoint(
    '../models/best_model_cropped_mouth_124_lm_sf_new.h5',          # Path where the model will be saved
    monitor='val_accuracy',   # Monitor validation accuracy for improvement
    verbose=1,                # Log when models are saved
    save_best_only=True,      # Only save a model if 'val_accuracy' has improved
    mode='max'                # 'max' means we look for the maximum validation accuracy
...
model = cropped_mouth_40_lm_attention()
reduce_lr = ReduceLROnPlateau(
    monitor='val_loss',
    factor=0.1,       # Factor by which the learning rate will be reduced. new_lr = lr * factor
    patience=10,      # Number of epochs with no improvement after which learning rate will be reduced.
...
checkpoint = ModelCheckpoint(
    '../models/best_model_cropped_mouth_40_lm_attention_new.h5',          # Path where the model will be saved
    monitor='val_accuracy',   # Monitor validation accuracy for improvement
    verbose=1,                # Log when models are saved
    save_best_only=True,      # Only save a model if 'val_accuracy' has improved
    mode='max'                # 'max' means we look for the maximum validation accuracy
...
model = cropped_mouth_124_lm_attention()
reduce_lr = ReduceLROnPlateau(
    monitor='val_loss',
    factor=0.1,       # Factor by which the learning rate will be reduced. new_lr = lr * factor
    patience=10,      # Number of epochs with no improvement after which learning rate will be reduced.
...
checkpoint = ModelCheckpoint(
    '../models/best_model_cropped_mouth_124_lm_attention_new.h5',          # Path where the model will be saved
    monitor='val_accuracy',   # Monitor validation accuracy for improvement
    verbose=1,                # Log when models are saved
    save_best_only=True,      # Only save a model if 'val_accuracy' has improved
    mode='max'                # 'max' means we look for the maximum validation accuracy
