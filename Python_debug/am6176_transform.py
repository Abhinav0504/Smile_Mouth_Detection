...
from errors import ArgumentError, CoordinateRangeError, InvalidEnumError
from typess import BBox, Detection, ImageTensor, Landmark, Rect
...
def image_to_tensor(
    ...
    """
    from PIL import Image
    print("\nSTEP 4: Image_To_Tesnor Function")
    print("---------------------------")

    # Print the first 10 and last 10 RGB values in 3-element array format before normalization
    print("\nInitial Image Tensor Values - First 10 RGB values:")
    initial_tensor_data = np.asarray(image, dtype=np.float32).reshape(-1, 3)
    for value in initial_tensor_data[:10]:
        print(value)

    print("\nInitial Image Tensor Values - Last 10 RGB values:")
    for value in initial_tensor_data[-10:]:
        print(value)
    ...
    print("\n   STEP 4.1: _normalize_to_image() results")
    print("     ---------------------------")
    print("     Normalized Image Mode:", img.mode)
    print("     Normalized Image Size:", img.size)

    print("\nInitial Image Tensor Values - First 10 Normalized values:")
    img1 = np.asarray(img, dtype=np.float32).reshape(-1, 3)
    for value in img1[:10]:
        print(value)

    print("\nInitial Image Tensor Values - Last 10 Normalized values:")
    for value in img1[-10:]:
        print(value)
    ...
    print(f"\n   STEP 4.2: ROI Scaled Size: {roi.size}")
    print("     ---------------------------")
    ...
    print(f"    roi_image size: {roi_image.size}")
    if img != image:
        img.close()

    roi_image_array = np.asarray(roi_image, dtype=np.float32)
    print(len(roi_image_array))

    print("\nFirst 10 values after perspective transformation (Python):")
    for value in roi_image_array.reshape(-1, 3)[:10]:
        print(value)

    print("\nLast 10 values after perspective transformation (Python):")
    for value in roi_image_array.reshape(-1, 3)[-10:]:
        print(value)
    ...
    if keep_aspect_ratio:
        ...
        # cv2.imwrite("Padded_image.jpg", roi_image_array)

        print(f"\nLength of roi_image_array after padding (Python): {len(roi_image_array.reshape(-1, 3))}")

        print("\nFirst 10 values after padding (Python):")
        for value in roi_image_array.reshape(-1, 3)[:10]:
            print(value)

        print("\nLast 10 values after padding (Python):")
        for value in roi_image_array.reshape(-1, 3)[-10:]:
            print(value)
        ...
        # Visualize the image after resizing
        plt.figure()
        plt.imshow(roi_image)
        ...
        if keep_aspect_ratio:
            print(f"    out_aspect = {out_aspect}, roi_aspect = {roi_aspect}")
            print(f"    new_width = {new_width}, new_height = {new_height}")
            if new_width != int(roi.width) or new_height != int(roi.height):
                print(f"    pad_x = {pad_x}, pad_y = {pad_y}")
            print(f"    Resized with aspect ratio. New size: {roi_image.size}")
    ...
    # cv2.imwrite("Resized_image.jpg", roi_image_array)

    print(f"\nLength of roi_image_array after padding and resizing (Python): {len(roi_image_array.reshape(-1, 3))}")

    print("\nFirst 10 values after padding and resizing (Python):")
    for value in roi_image_array.reshape(-1, 3)[:10]:
        print(value)

    print("\nLast 10 values after padding and resizing (Python):")
    for value in roi_image_array.reshape(-1, 3)[-10:]:
        print(value)
    ...
    print("\n   STEP 4.6: Applying Value Range Transform")
    print("     ---------------------------")
    ...
    tensor_data += min_val
    print("    Applying value range transform: min_val =", min_val, ", max_val =", max_val)
    print("    Image size after value range transform:", tensor_data.shape)
    print("    First pixel value after transform:", tensor_data[0, 0])

    # Python: Print first 10 transformed values
    print("\nFirst 10 Value Range Transformed values (Python):")
    for value in tensor_data.reshape(-1, 3)[:10]:
        print(value)

    # Python: Print last 10 transformed values
    print("\nLast 10 Value Range Transformed values (Python):")
    for value in tensor_data.reshape(-1, 3)[-10:]:
        print(value)
