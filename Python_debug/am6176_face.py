...
# Set full output width
sys.stdout.reconfigure(line_buffering=True)
# np.set_printoptions(threshold=sys.maxsize)  # Full numpy array output
...
class FaceDetection1:
    ...
    #     print first and last 10 anchors
        print("\nFirst 10 anchors:")
        for anchor in self.anchors[:10]:
            print(anchor)
    ...
    def __call__(
        ...
        # Reshape to a flat array for easier indexing
        sigmoid_scores_flat = scores.reshape(-1)

        # Print first 5 and last 5 values
        print("\nFirst 10 sigmoid scores:", sigmoid_scores_flat[:10])
        print("Last 10 sigmoid scores:", sigmoid_scores_flat[-10:])

        # print("\nFirst 5 sigmoid scores:", scores[:5])
        # print("Last 5 sigmoid scores:", scores[-5:])
        ...
        # print(f"Print all detections:{detections}")
    ...
    def _decode_boxes(self, raw_boxes: np.ndarray) -> np.ndarray:
        ...
        print(f" raw_boxes.shape[-1]: { raw_boxes.shape[-1]}")
        print(f"####Number of points: {num_points}")

        # Step 1: Reshape and Scale
        boxes = raw_boxes.reshape(-1, num_points, 2) / scale
        print("\n\tStep 6.1 - Reshaped boxes shape:", boxes.shape)
        print("\n\t---------------------------------")


        print(f"\tLength of Raw boxes: {len(raw_boxes[0])}")

        # Debug: Print raw boxes before decoding
        print("\tRaw Boxes (sample 5):")
        for i in range(min(5, len(raw_boxes[0]))):
            print(f"  Box {i}: {raw_boxes[0][i]}")

        # Step 2: Anchor Adjustment
        # print("\tboxes[:5] (first 5 rows):")
        # print(boxes[:5])
        ...
        print(f"\tself.anchors: {len(self.anchors)}")
        for i in range(2, num_points):
            boxes[:, i] += self.anchors

        # Debug: Print adjusted anchors for first 5 boxes
        # print("\tAdjusted Anchors - Full output sample:")
        # print(boxes[:5])
        ...
        # Debug: Print decoded bounding boxes
        # print("\tDecoded Boxes:")
        # for i, box in enumerate(boxes[:5]):
        #     print(f"  Box {i}: xmin={box[0][0]}, ymin={box[0][1]}, xmax={box[1][0]}, ymax={box[1][1]}")

        print("\tFinal Adjusted Boxes before returning")
        print(boxes[:5])
    ...
    def _convert_to_detections(
        ...
        print("\n\tStep 6.2 - Convert To Detections:")
        print("\n\t---------------------------------")
        ...
        print("\n\tScore threshold mask:", len(score_above_threshold[0]))
        ...
        for i, detection in enumerate(detections[:5]):
            print(f"\n\tDetection {i} - Box: {detection.data.flatten()}, \nScore: {detection.score}")
        if len(detections) > 5:
            print("\n\tAdditional detections exist, total count:", len(detections))
