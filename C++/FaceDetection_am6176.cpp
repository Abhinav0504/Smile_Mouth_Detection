...
//    std::vector<cv::Rect> decoded_boxes;
...
//    std::cout << "Anchor Adjustment:" << std::endl;
//    for (int i = 0; i < num_boxes; ++i) {
//        // Add anchors to boxes[:, 0]
//        boxes[i][0].first += anchors[i].first;  // Adjust x-coordinate of the first point
//        boxes[i][0].second += anchors[i].second;  // Adjust y-coordinate of the first point
//
//        // Add anchors to boxes[:, 2:] (excluding boxes[:, 1])
//        for (int j = 2; j < num_points; ++j) {
//            boxes[i][j].first += anchors[i].first;  // Adjust x-coordinate
//            boxes[i][j].second += anchors[i].second;  // Adjust y-coordinate
//        }
//    }
//
//    // Debug: Print adjusted anchors for the first 5 boxes
//    std::cout << std::endl << "Adjusted Anchors - Full output sample:" << std::endl;
//    for (int i = 0; i < std::min(5, num_boxes); ++i) {
//        std::cout << std::endl << "  Box " << i << ": [";
//        for (int j = 0; j < num_points; ++j) {
//            std::cout << "[" << boxes[i][j].first << ", " << boxes[i][j].second << "]";
...
//    std::cout << "Decoded Boxes:" << std::endl;
//    for (int i = 0; i < num_boxes; ++i) {
//        // Center coordinates (boxes[:, 0])
...
//        float xmin = x_center - half_width;
//        float ymin = y_center - half_height;
//        float xmax = x_center + half_width;
//        float ymax = y_center + half_height;
//
//        // Debug: Print decoded bounding box
//        if (i < 5) {
//            std::cout << "  Box " << i << ": xmin=" << xmin << ", ymin=" << ymin
//                      << ", xmax=" << xmax << ", ymax=" << ymax << std::endl;
//        }
//
//        decoded_boxes.push_back(cv::Rect2f(cv::Point2f(xmin, ymin), cv::Point2f(xmax, ymax)));
//
////        // Debug: Print the first 5 boxes for verification
////        if (i < 5) {
////            std::cout << "  Decoded Box " << i << ": xmin=" << xmin << ", ymin=" << ymin
////                      << ", xmax=" << xmax << ", ymax=" << ymax << std::endl;
////        }
//    }
//
//    // Debug: Print size of the returned vector
//    std::cout << "Total Decoded Boxes inside function: " << decoded_boxes.size() << std::endl;
//
//// Debug: Print first 5 and last 5 boxes
//    std::cout << "Decoded Boxes (First 5 and Last 5):" << std::endl;
//
//// First 5 boxes
//    std::cout << "First 5 Decoded Boxes:" << std::endl;
//    for (int i = 0; i < std::min(5, static_cast<int>(decoded_boxes.size())); ++i) {
//        const auto& box = decoded_boxes[i];
//        std::cout << "  Box " << i << ": xmin=" << box.x << ", ymin=" << box.y
//                  << ", xmax=" << (box.x + box.width) << ", ymax=" << (box.y + box.height) << std::endl;
//    }
//
//// Last 5 boxes
//    std::cout << "Last 5 Decoded Boxes:" << std::endl;
//    for (int i = std::max(0, static_cast<int>(decoded_boxes.size()) - 5); i < decoded_boxes.size(); ++i) {
//        const auto& box = decoded_boxes[i];
//        std::cout << "  Box " << i << ": xmin=" << box.x << ", ymin=" << box.y
//                  << ", xmax=" << (box.x + box.width) << ", ymax=" << (box.y + box.height) << std::endl;
//    }
...
std::vector<Detection> FaceDetection::convert_to_detections(
    ...
    auto is_valid = [](const std::vector<std::pair<float, float>>& box) -> bool {
        return (box[1].first > box[0].first) && (box[1].second > box[0].second); // width > 0 and height > 0
    };

    for (size_t i = 0; i < boxes.size() && i < scores.size(); ++i) {
        if (scores[i] > MIN_SCORE) { // Check if score is above threshold
            const auto& box = boxes[i];
            if (is_valid(box)) { // Check if the box is valid
                // Convert box to cv::Point2f format
                std::vector<cv::Point2f> points = {
                        cv::Point2f(box[0].first, box[0].second), // xmin, ymin
                        cv::Point2f(box[1].first, box[1].second)  // xmax, ymax
                };
                // Add Detection to the result
                detections.emplace_back(points, scores[i]);
            }
        }
    }
    return detections;
